...
object IabEnrichment extends ParseableEnrichment {
   ...
   */
  def apply[F[_]: Monad: CreateIabClient](
    conf: IabConf
  ): F[IabEnrichment] =
    CreateIabClient[F].create(conf.ipFile._2, conf.excludeUaFile._2, conf.includeUaFile._2)
...
trait CreateIabClient[F[_]] {
  def create(ipFile: String, excludeUaFile: String, includeUaFile: String): F[IabClient]
}
...
object CreateIabClient {
  ...
  implicit def evalCreateIabClient: CreateIabClient[Eval] = new CreateIabClient[Eval] {
    def create(ipFile: String, excludeUaFile: String, includeUaFile: String): Eval[IabClient] =
      Eval.later {
        new IabClient(new File(ipFile), new File(excludeUaFile), new File(includeUaFile))
      }
  }
...
private[enrichments] final case class IabDatabase(name: String, uri: URI, db: String)
