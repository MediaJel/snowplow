...
class ApiRequestEnrichmentIntegrationTest extends Specification {
  ...
  def e1 = {
    val enrichment = ApiRequestEnrichment.parse(IntegrationTests.configuration, SCHEMA_KEY)
      .map(_.enrichment)
      .toEither
  ...
  def e2 = {
    val enrichment = ApiRequestEnrichment.parse(IntegrationTests.configuration2, SCHEMA_KEY)
      .map(_.enrichment)
      .toEither
    val event = new EnrichedEvent
    event.setApp_id("lookup test")
